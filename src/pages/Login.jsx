...
export default function Login() {
  ...
  // Google / Single Sign On Handler (Direct SSO Sign-in without public account list)
  const handleGoogleSSO = () => {
    toast.loading('Authenticating via Google Workspace SSO...', { duration: 1200 });
    ...
    setTimeout(() => {
      ...
      const res = login(targetEmail, 'sso_google_valid_token', activeTab);

      if (res.success && res.user) {
        toast.success(`Signed in via Google SSO (${targetEmail})!`);
        if (res.user.role === 'super_admin') {
          navigate('/admin-home', { replace: true });
        } else if (res.user.role === 'warden') {
          navigate('/hostel-dashboard', { replace: true });
